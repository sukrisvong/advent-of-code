...
def initialize_grid():
    return [[SPACE for _ in range(GRID_WIDTH)] for _ in range(GRID_HEIGHT)]

def add_wall(grid, location):
    grid[location[1]][location[0]] = WALL
    return grid
...
for location in locations:
    add_wall(grid, location)
    path_found, path_length = get_length_of_shortest_path(grid)
    if not path_found:
        print(location)
        break
